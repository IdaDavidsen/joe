...
db.serialize(function () {
  ...
  db.run(
    "CREATE TABLE if not exists keys (id integer primary key, value text not null, datetime integer)"
  );
});
...
app.get("/authenticate", (req, res) => {
  ...
  db.serialize(function () {
    db.run(
      "INSERT INTO keys (value, datetime) VALUES (?, ?)",
      [authorizationKey, Date.now()],
      function (err) {
        if (err) {
          console.error(err);
